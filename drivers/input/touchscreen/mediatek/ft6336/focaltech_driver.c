...
#if CTP_ADD_HW_INFO
static char tp_string_version[100];
#endif

#if CTP_OPEN_SHORT_TEST
extern void fts_create_openshort_proc();
#endif
...
extern void focal_Gesture_Fucntion_Proc_File(void);

char focal_gesture_value = 'Z';
char focal_gesture_onoff = '0';
char focal_glove_onoff = '0';
const char focal_gesture_type[]=GTP_GESTURE_TPYE_STR; 	 
const char focal_glove_support_flag=GTP_GLOVE_SUPPORT_ONOFF;
const char focal_gesture_support_flag=GTP_GESTURE_SUPPORT_ONOFF;
const char focal_verson[] =GTP_DRIVER_VERSION;
char focal_glove_support_flag_changed=NULL;;
char focal_gesture_support_flag_changed=NULL;
#endif
...
#ifdef FTS_GESTRUE
int focal_proc_gesture_data_read(char *page, char **start, off_t off, int count, int *eof, void *data)
  { 
	 //GTP_INFO("proc_gesture_data_read=%d\n",gtp_gesture_value);
	 return sprintf(page,"%c\n",focal_gesture_value);	 
  }
  
 int focal_proc_gesture_data_write(struct file *file, const char *buffer, unsigned long count, void *data)
  {    
	 sscanf(buffer,"%c",&focal_gesture_value);	 
	  return count;
  }
  
int focal_proc_gesture_type_read(char *page, char **start, off_t off, int count, int *eof, void *data)
  {
...
int focal_proc_gesture_type_write(struct file *file, const char *buffer, unsigned long count, void *data)
  {    
	  
	  return -1;
  }
  
int focal_proc_gesture_onoff_read(char *page, char **start, off_t off, int count, int *eof, void *data)
  {
	  if(NULL==focal_gesture_support_flag_changed) return sprintf(page,"%c\n",focal_gesture_support_flag);
	  else return sprintf(page,"%c\n",focal_gesture_onoff); 
 
  }
  
int focal_proc_gesture_onoff_write(struct file *file, const char *buffer, unsigned long count, void *data)
  {    
	  sscanf(buffer,"%c",&focal_gesture_onoff);
	  focal_gesture_support_flag_changed=1;
	   return count;
  }
int focal_proc_glove_onoff_read(char *page, char **start, off_t off, int count, int *eof, void *data)
  {
	  if(NULL==focal_glove_support_flag_changed) return sprintf(page,"%c\n",focal_glove_support_flag);
	 else return sprintf(page,"%c\n",focal_glove_onoff);
 
  }
  
  int focal_proc_glove_onoff_write(struct file *file, const char *buffer, unsigned long count, void *data)
  {    
	  sscanf(buffer,"%c",&focal_glove_onoff);   
	  focal_glove_support_flag_changed=1;
...
int focal_proc_version_gesture_read(char *page, char **start, off_t off, int count, int *eof, void *data)
	 {
		 return sprintf(page,"%s\n",focal_verson);

	 }
	 
int focal_proc_version_gesture_write(struct file *file, const char *buffer, unsigned long count, void *data)
	 {	  
		 return count;
	 }

 void focal_Gesture_Fucntion_Proc_File(void)
  {  
	   ...
	   {
   		 FOCAL_DEBUG("ctp_gesture_var_proc create failed\n");
	   } 
	   else 
	   {
		ctp_gesture_var_proc->read_proc = focal_proc_gesture_data_read;
		ctp_gesture_var_proc->write_proc = focal_proc_gesture_data_write;
		FOCAL_DEBUG("ctp_gesture_var_proc create success\n");
	   }
  
	   ctp_gesture_type_proc = create_proc_entry("gesture_type", 0444, ctp_device_proc);
	   if (ctp_gesture_type_proc == NULL) 
	   {
 		   FOCAL_DEBUG("ctp_gesture_type_proc create failed\n");
	   } 
	   else 
	   {
		ctp_gesture_type_proc->read_proc = focal_proc_gesture_type_read;
		ctp_gesture_type_proc->write_proc = focal_proc_gesture_type_write;
		FOCAL_DEBUG("ctp_gesture_type_proc create success\n");
	   }
	   
	   ctp_gesture_onoff_proc = create_proc_entry("gesture_onoff", 0666, ctp_device_proc);
	   if (ctp_gesture_onoff_proc == NULL) 
	   {
   		 FOCAL_DEBUG("ctp_gesture_onoff_proc create failed\n");
	   } 
	   else 
	   {
		ctp_gesture_onoff_proc->read_proc = focal_proc_gesture_onoff_read;
		ctp_gesture_onoff_proc->write_proc = focal_proc_gesture_onoff_write;
		FOCAL_DEBUG("ctp_gesture_onoff_proc create  success\n");
	   }  
	   ...
	   {
		ctp_glove_onoff_proc->read_proc = focal_proc_glove_onoff_read;
		ctp_glove_onoff_proc->write_proc = focal_proc_glove_onoff_write;
		FOCAL_DEBUG("ctp_glove_onoff_proc create success\n");
	   }
	...
	{
		FOCAL_DEBUG("create_proc_entry version failed\n");
	  } 
	  else 
	  {
		ctp_version_proc->read_proc = focal_proc_version_gesture_read;
		ctp_version_proc->write_proc = focal_proc_version_gesture_write;
		FOCAL_DEBUG("create_proc_entry version success\n");
	  }
...
static void check_gesture(int gesture_id)
{
	...
	FOCAL_DEBUG("kaka gesture_id==0x%x\n ",gesture_id);

	switch(gesture_id)
	{
		case GESTURE_DOUBLECLICK:
			focal_gesture_value = 'K';
...
static int ft5x0x_read_Touchdata(void)
{
	...
	if (ret < 0)
	{
		FOCAL_DEBUG( "%s read touchdata failed.\n", __func__);
		return ret;
	}
	/* FW ?¡À?¨®??3?¨º?¨º? */
	if (0x24 == buf[0])
...
{ 
	...
	do
	{
		...
		FOCAL_DEBUG("in event handler");
#ifdef FTS_GESTRUE
	if(focal_gesture_onoff == '1'){
		//i2c_smbus_read_i2c_block_data(i2c_client, 0xd0, 1, &state);
		FOCAL_DEBUG("in event handler state :%d", state);
...
int fts_read_vendor_info(struct tp_vendor_info *tp_info, struct i2c_client *client)
{
	int ret = -1;
	int i = 0;
	if(NULL == tp_info || client == NULL)
		return ret;

	//read fw_version
	i2c_smbus_read_i2c_block_data(client,FT_REG_FW_VER,1,&tp_info->fw_version);
	i2c_smbus_read_i2c_block_data(client,FT_REG_VENDOR_ID,1,&tp_info->vendor_id);
	//read ic_type
	i2c_smbus_read_i2c_block_data(client,FT_REG_CHIP_ID,1,&tp_info->chip_id);
	for(i=0;i<sizeof(fts_updateinfo)/sizeof(struct Upgrade_Info);i++)
	{
		if(tp_info->chip_id==fts_updateinfo[i].CHIP_ID)
		{
			sprintf(tp_info->ic_name, "%s", fts_updateinfo[i].FTS_NAME);
			break;
		}
	}
	if(i >= sizeof(fts_updateinfo)/sizeof(struct Upgrade_Info))
	{
		sprintf(tp_info->ic_name, "%s", "tp_vendor_error");
	}
...
reset_proc:   
	...
	FOCAL_DEBUG("FTS:err %d,data:%d\n", err,data);
	if(err< 0 || data!=0)// reg0 data running state is 0; other state is not 0
	{
		FOCAL_DEBUG("I2C transfer error, line: %d\n", __LINE__);
    ...
    fts_read_vendor_info(&tp_info, i2c_client);
#if CTP_ADD_HW_INFO
		sprintf(tp_string_version,"%s, vendor:0x%x, fw_ver:0x%x", tp_info.ic_name, tp_info.vendor_id, tp_info.fw_version);
	...
	focal_Gesture_Fucntion_Proc_File();
...
static void tpd_resume( struct early_suspend *h )
{
	...
	FOCAL_DEBUG("TPD wake up\n");

#ifdef FTS_GESTRUE
		FOCAL_DEBUG("write gesture\n");
...
#ifdef FTS_GESTRUE
	if(focal_gesture_onoff == '1'){
		FOCAL_DEBUG("in suspend write reg");
		fts_write_reg(i2c_client, 0xd0, 0x01);
