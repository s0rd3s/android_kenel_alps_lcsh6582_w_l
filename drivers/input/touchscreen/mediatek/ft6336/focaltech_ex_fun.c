...
u8 *I2CDMABuf_pa = NULL;

extern struct Upgrade_Info fts_updateinfo_curr;
#if CTP_OPEN_SHORT_TEST
extern struct i2c_client *global_i2c_client;
#endif
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	...
	if (((CTPM_FW[fw_len - 8] ^ CTPM_FW[fw_len - 6]) == 0xFF
		...
		if (i_ret != 0)
			FOCAL_DEBUG("%s:upgrade failed. err.\n",__func__);
		else if(fts_updateinfo_curr.AUTO_CLB==AUTO_CLB_NEED)
			fts_ctpm_auto_clb(client);	/*start auto CLB */

	} else {
		FOCAL_DEBUG("%s:FW format error\n", __func__);
		return -EBADFD;
	}
...
u8 fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	*/
	...
	if (reg_val[0] == 0x79 && reg_val[1] == 0x18)
		FOCAL_DEBUG("Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0], reg_val[1]);
	else
		return 0;

	auc_i2c_write_buf[0] = 0xcd;
	fts_i2c_Read(client, auc_i2c_write_buf, 1, reg_val, 1);
	FOCAL_DEBUG("bootloader version = 0x%x\n", reg_val[0]);
	...
	msleep(10);
	FOCAL_DEBUG("uc_panel_factory_id = 0x%x\n",buf[4]);
...
u8 fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	u8 w_buf[4], r_buf[2];
    u32 i = 0,j=0;
	u8 ft_upgrade_loop = 30;
	FOCAL_DEBUG("in project setting\n");
	...
	for (i = 0, j = 0; i < ft_upgrade_loop; i++)
	{
		...
		fts_write_reg(client, w_buf[0], 1);
		msleep(5);
		...
		fts_i2c_Read(client, w_buf, 4, r_buf, 2);

		if (r_buf[0] != 0x79 || r_buf[1] != 0x79)
		{
			FOCAL_DEBUG("r_buf[0,1] = %x,%x\n", r_buf[0],r_buf[0]);
			continue;
		}
		else
			break;
    ...
    fts_i2c_Read(client, buf, 4, buf, 128);
    msleep(10);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	{
		FOCAL_DEBUG("ic vendor %x, file vendor %x\n", uc_tp_vendor_id,uc_host_vendor_id);
		if(0x51 == fts_ctpm_update_project_setting(client))
		{
			ic_fw_crash = 1;
		}
		else
	...
	if (/*the firmware in touch panel maybe corrupted */
		...
		/*the firmware in host flash is new, need upgrade */
	     uc_tp_fm_ver < uc_host_fm_ver||ic_fw_crash == 1
	    ) {
		msleep(100);
		...
		if (i_ret == 0)	{
			msleep(300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			FOCAL_DEBUG("upgrade to new version 0x%x\n",uc_host_fm_ver);
		} else {
			FOCAL_DEBUG("upgrade failed ret=%d.\n", i_ret);
			return -EIO;
		}
	}
			  ...
			  u32 dw_lenth)
{
	...
	{
	    msleep(100);
		FOCAL_DEBUG("[FTS] Step 1:Reset  CTPM\n");
		/*********Step 1:Reset  CTPM *****/
		/*write 0xaa to register 0xfc */
		...
		/*********Step 2:Enter upgrade mode *****/
		FOCAL_DEBUG("[FTS] Step 2:Enter upgrade mode \n");
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			...
			FOCAL_DEBUG("[FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0], reg_val[1]);
			break;
	...
	FOCAL_DEBUG("Step 4:erase app and panel paramenter area\n");
	...
	bt_ecc = 0;
	FOCAL_DEBUG("Step 5:write firmware(FW) to ctpm flash\n");
	...
	for (j = 0; j < packet_number; j++) {
		...
		FOCAL_DEBUG("write bytes:0x%04x\n", (j+1) * FTS_PACKET_LENGTH);
	...
	FOCAL_DEBUG("Step 6: read out checksum\n");
	...
	FOCAL_DEBUG("Step 7: reset the new FW\n");
	auc_i2c_write_buf[0] = 0x07;
	fts_i2c_Write(client, auc_i2c_write_buf, 1);
	...
	const char __user *buff, unsigned long len, void *data)
{
	...
	case PROC_READ_REGISTER:
		writelen = 1;
		FOCAL_DEBUG("%s:register addr=0x%02x\n", __func__, writebuf[1]);
...
static int ft5x0x_GetInISize(char *config_name)
{
	...
	FOCAL_DEBUG("ini file path:%s", filepath);

	if (NULL == pfile)
		pfile = filp_open(filepath, O_RDONLY, 0);

	if (IS_ERR(pfile)) {
		FOCAL_DEBUG("error occured while opening file %s.\n", filepath);
		return -EIO;
	}
...
static int ft5x0x_get_testparam_from_ini(char *config_name)
{
	...
	FOCAL_DEBUG("inisize = %d \n ", inisize);
	if (inisize <= 0) {
		FOCAL_DEBUG("%s ERROR:Get firmware size failed\n",
					__func__);
		return -EIO;
	}
	...
	if(filedata ==NULL)
	{
		FOCAL_DEBUG("alloc mem faild!\n");
		return -EIO;
	}
		
	if (ft5x0x_ReadInIData(config_name, filedata)) {
		FOCAL_DEBUG("%s() - ERROR: request_firmware failed\n",
					__func__);
		kfree(filedata);
		return -EIO;
	} else {
		FOCAL_DEBUG("ft5x0x_ReadInIData successful\n");
	}
...
int focal_test_i2c_Write(unsigned char *writebuf, int writelen)
{
	if(NULL == global_i2c_client)
	{
		FOCAL_DEBUG("client is null");
		return -1;
	}else{
		return fts_i2c_Write(global_i2c_client,writebuf, writelen);
	}
}
int focal_test_i2c_Read(unsigned char *writebuf, int writelen, unsigned char *readbuf, int readlen)
{
	if(NULL == global_i2c_client)
	{
		FOCAL_DEBUG("client is null");
		...
		return fts_i2c_Read(global_i2c_client, writebuf, writelen,readbuf,readlen);
	}
}

static int ft5x0x_ftsscaptest_store(const char *buf, size_t count)
{
    	char cfgname[128];
	...
	FOCAL_DEBUG("cfgname:%s, filename count:%d", cfgname, count);
	Init_I2C_Write_Func(focal_test_i2c_Write);
	Init_I2C_Read_Func(focal_test_i2c_Read);
	if(ft5x0x_get_testparam_from_ini(cfgname) <0)
		FOCAL_DEBUG("get testparam from ini failure\n");
	else {

		if(true == StartTestTP())
		{
			test_result = 1;
			FOCAL_DEBUG("tp test pass\n");
		}
		else
		{
			test_result = 0;
			FOCAL_DEBUG("tp test failed\n");
		}
		FreeTestParamData();
	}
...
int fts_opentest_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int cnt= 0;
    int result = 0;
   ...
   if(off > 0)
    ...
    result = ft5x0x_ftsscaptest_store("ft6336_oufei.ini",  strlen("ft6336_oufei.ini"));
    *eof = 1;
    return sprintf(&page[0], "result=%d", result); //cnt
}

int fts_opentest_write(struct file *file, const char *buffer, unsigned long count, void *data)
{    
    u16 i = 0;
    mdelay(5);
...
int fts_shorttest_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    FOCAL_DEBUG("shorttest read");
...
int fts_shorttest_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    FOCAL_DEBUG("tpd_shorttest write");
    return -1;
}


void fts_create_openshort_proc()
{
	struct proc_dir_entry *fts_ito_test = NULL;
	struct proc_dir_entry *opentest = NULL;
	struct proc_dir_entry *shorttest = NULL;

	fts_ito_test = proc_mkdir("ctp_test", NULL);
	opentest = create_proc_entry("opentest", 0777, fts_ito_test);
	shorttest = create_proc_entry("shorttest", 0777, fts_ito_test);

    if (NULL==opentest) 
    {
        FOCAL_DEBUG("create_proc_entry ITO TEST DEBUG failed\n");
    } 
    else 
    {
        opentest->read_proc = fts_opentest_read;
        opentest->write_proc = fts_opentest_write;
        FOCAL_DEBUG("create_proc_entry ITO TEST DEBUG OK\n");
    }
    if (NULL==shorttest) 
    {
        FOCAL_DEBUG("create_proc_entry ITO TEST DEBUG failed\n");
    ...
    {
        shorttest->read_proc = fts_shorttest_read;
        shorttest->write_proc = fts_shorttest_write;
        FOCAL_DEBUG("create_proc_entry ITO TEST DEBUG OK\n");
    }
