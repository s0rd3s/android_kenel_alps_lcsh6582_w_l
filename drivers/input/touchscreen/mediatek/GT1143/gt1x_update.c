...
//      int check_state;
	...
	GTP_DEBUG("gt1x_update_firmware 1, %d", ret);
	if (force_update == 1)
		GTP_INFO("INTO FORCE UPDATE");
	else{
		if (ret) {
			goto gt1x_update_exit;
		}
	...
	GTP_DEBUG("gt1x_update_firmware 2");
	...
	GTP_DEBUG("gt1x_update_firmware 3");
...
int gt1x_update_prepare(char *filename)
{
	...
	while (retry > 0) {
		...
		if (update_info.firmware == NULL) {
			GTP_INFO("Alloc %d bytes memory fail.", sizeof(struct fw_info));
			continue;
		} else {
			GTP_INFO("Alloc %d bytes memory success.", sizeof(struct fw_info));
			break;
		}
	}
	if (retry <= 0) {
