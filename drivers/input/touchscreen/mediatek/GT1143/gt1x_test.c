...
int goodix_atoi(const char *str)
{
...
int parse_config(unsigned char *config)
{
	...
	DEBUG("sen %d, drv %d", sys.driver_num, sys.sensor_num);
...
int read_raw_data(unsigned short *data, int len)
{
	...
	u16 rawdata[sys.sensor_num * sys.driver_num];

	u8 buf[sys.sensor_num * sys.driver_num * 2];
...
int disable_hopping(unsigned char *config, int len, int flag)
{
	...
	do {
		...
		if (!memcmp(&config[1], &tmp_config[1], len - 5)) {
...
void write_test_params(u8 * config)
{

	u8 data[_DRV_TOTAL_NUM + _SEN_TOTAL_NUM], i, m;
	...
	memset(data, 0xFF, _DRV_TOTAL_NUM + _SEN_TOTAL_NUM);
	...
	if (!strncmp((const char *)gt1x_version.product_id, "1143", 4)) {
		...
		for (i = 0, m = 0; i < 20; i++) {
			...
			if ((config[drv_offest + i] >= 20) && (config[drv_offest + i] < 40))	//drv channel
				data[_SEN_TOTAL_NUM + m++] = gt1143_sen_map[config[drv_offest + i]];
		...
		for (i = 0, m = 0; i < _SEN_TOTAL_NUM; i++) {
		...
		for (i = 0, m = 0; i < _DRV_TOTAL_NUM; i++) {

			if (config[drv_offest + i] == 0xFF)

				break;

			data[_SEN_TOTAL_NUM + m++] = config[drv_offest + i];

		}

	}

	for (i = 0; i < _DRV_TOTAL_NUM + _SEN_TOTAL_NUM; i++) {
	...
	i2c_write_data(0x880E, &data[_SEN_TOTAL_NUM], _DRV_TOTAL_NUM);

	i2c_write_data(0x882E, data, _SEN_TOTAL_NUM);

	i2c_write_data(0x884E, &chksum, 1);

	DEBUG("write test parameters sen%d*drv%d", _SEN_TOTAL_NUM, _DRV_TOTAL_NUM);
}
...
static int _calculate_short_resistance_with_gnd(u16 short_code, u8 pos)
{
	...
	if ((short_code & (0x8000)) == 0)	//short to GND
		r = (52662850) / (short_code & (~0x8000)) - 40 * FLOAT_AMPLIFIER;	//52662.85/code-40
	else			//short to VDD
		//r = ( 1/(((float)(short_code&(~0x8000)))/0.9*0.7/1024/(sys_avdd-0.9)/40) ) -40;
		r = 4 * 9 * 1024 * (sys_avdd - 9) * FLOAT_AMPLIFIER / (short_code & (~0x8000) * 7) - 40 * FLOAT_AMPLIFIER;
	...
	if (short_code < r_th) {
		...
		upload_short_data[totals * 4 + 2] = _DRV_TOTAL_NUM + 1;	//short with GND
...
static int _calculate_short_resistance(strShortRecord r_data, unsigned short short_r_th, unsigned char flag)
{
	...
	if (j > (_DRV_TOTAL_NUM + _SEN_TOTAL_NUM)) {
...
int short_test_analysis(unsigned char *result, int t_size)
{
	...
	offest += (_DRV_TOTAL_NUM + _SEN_TOTAL_NUM) * sizeof(unsigned short);

	tmp = (unsigned char *)(&large_buf[offest]);

	if ((short_flag & 0x08) == 0x08) {

		length = (_DRV_TOTAL_NUM + _SEN_TOTAL_NUM) * 2;
	...
	for (i = 0; i < short_status[0]; i++) {
		...
		length = _SHORT_HEAD + _DRV_TOTAL_NUM * 2 + 2;

		i2c_read_data(result_addr, tmp, length);

		for (j = i + 1; j < _DRV_TOTAL_NUM; j++) {

			short_code = (tmp[_SHORT_HEAD + j * 2] << 8) + tmp[_SHORT_HEAD + j * 2 + 1];
	...
	for (i = 0; i < short_status[1]; i++)	//RX
	{
		...
		length = _SHORT_HEAD + _SEN_TOTAL_NUM * 2 + 2;

		i2c_read_data(result_addr, tmp, length);

		DEBUG_ARRAY(tmp, length);

		for (j = 0; j < _SEN_TOTAL_NUM; j++) {
			...
			if (short_code > gt900_short_threshold) {
				...
				r_data.position = (r_data.master & 0x7f) + _DRV_TOTAL_NUM;
...
int ini_read_int(char *file, const char *key)
{
	...
	value = goodix_atoi((const char *)tmp);
	...
	if (ret < 0) {
		...
		if (ret & _GT_SHORT) {
			...
			for (i = 0; i < result[0]; i++) {
				...
				else if (result[1 + i * 4] == (_DRV_TOTAL_NUM + 1)) {
				...
				else if (result[2 + i * 4] == (_DRV_TOTAL_NUM + 1)) {
...
/*******************************************************
create proc node for open_short test
*******************************************************/
extern int tpd_halt;
...
static ssize_t gtp_short_test_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
   ...
   if(result == NULL){
   	   ret = 0;
	   GTP_INFO("can't alloct memory");
   }
   else{
   	   //if short test ok ret=0,else ret=1;
	   ret=_open_short_test(result);
	   ret = (ret == 0?1:0);
   }
   free(result);

	GTP_INFO("test result %x", ret);
    len = sprintf(ptr, "result=%x\n", ret);
    *ppos += len;
    return len;
}
...
void gt1x_openshort_test_create_entry(void)
{
...
/*******************************************************
create proc node for open_short test end
*******************************************************/
...
{
	gt1x_openshort_test_create_entry();
	return 0;
}
