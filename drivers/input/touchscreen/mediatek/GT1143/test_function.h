...
#define FORMAT_PATH(path,mdir) do{\
                               struct timex txc;\
                               struct rtc_time tm;\
                               do_gettimeofday(&(txc.time));\
                               rtc_time_to_tm(txc.time.tv_sec,&tm);\
                               sprintf((char*)path,"%stestdata_%04d%02d%02d%02d%02d%02d.csv",mdir,(tm.tm_year+1900),(tm.tm_mon + 1),tm.tm_mday,tm.tm_hour,tm.tm_min,tm.tm_sec);\
                              }while(0)
...
s32 _open_short_test(unsigned char *short_result); 
#else
#include <time.h>
#include <math.h>
#include <stdio.h>
...
#define FORMAT_PATH(path,mdir) do{\
                      ...
                      sprintf((char*)path,"%stestdata_%04d%02d%02d%02d%02d%02d.csv",mdir,(1990 + tm->tm_year),1+tm->tm_mon,tm->tm_mday,tm->tm_hour,tm->tm_min,tm->tm_sec);\
                     }while(0)
