...
#if CTP_ADD_HW_INFO
char tp_string_version[40];
int Module_ID = 0;
#endif

#if GTP_GESTURE_WAKEUP
extern char gt1x_gtp_gesture_onoff;
#endif
...
static int gt1x_debug_write_proc(struct file *file, const char *buffer, size_t count, loff_t * ppos)
{
	s32 ret = 0;
...
s32 gt1x_i2c_read_dbl_check(u16 addr, u8 * buffer, s32 len)
{
	...
	if (len > 16) {
		GTP_ERROR("i2c_read_dbl_check length %d is too long, do not beyond %d", len, sizeof(buf));
		return ERROR;
	}
...
s32 gt1x_init(void)
{
	...
	GTP_INFO("gt1x_init_failed %d, gt1x_break_init %d\n", gt1x_init_failed, gt1x_break_init);
	if((gt1x_init_failed== (u8)0)&&(gt1x_break_init== (u8)1))
	{
		GTP_INFO("not gt1143");
		return NOT_GT1143;
	}
	...
	else
	{
	    ...
	    sprintf(tp_string_version,"IC VERSION:GT%s_%04X_%04X, sensor_id=%x, cfg_version:0x%x", gt1x_version.product_id, gt1x_version.patch_id >> 8, gt1x_version.mask_id >> 8, gt1x_version.sensor_id, gt1x_config[0]);
