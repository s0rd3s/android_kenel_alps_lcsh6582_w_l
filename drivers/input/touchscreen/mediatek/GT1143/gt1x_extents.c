...
char gt1x_gtp_gesture_coordinate[60]= {0X8140 >> 8, 0X8140 & 0xFF};
char gt1x_gtp_gesture_value = 0;
char gt1x_gtp_gesture_onoff = '0';
char gt1x_gtp_glove_onoff = '0';
	...
	if(gt1x_gtp_gesture_onoff == '1')
	{
		if (DOZE_ENABLED == gesture_doze_status) {
			...
			if (ret == 0 && doze_buf[0] != 0) {
				...
				{
					input_report_key(dev, KEY_WAKE_GESTURE, 1);
					input_sync(dev);
					input_report_key(dev, KEY_WAKE_GESTURE, 0);
					input_sync(dev);
				}
				return 1;
			}
...
//----------------------gesture related interface for jni------------
const char gt1x_gtp_gesture_type[]=GTP_GESTURE_TPYE_STR;
static const char gt1x_gtp_glove_support_flag=GTP_GLOVE_SUPPORT_ONOFF;
static const char gt1x_gtp_gesture_support_flag=GTP_GESTURE_SUPPORT_ONOFF;
static const char gt1x_gtp_verson[] =GTP_PROC_DRIVER_VERSION;
static  char gt1x_gtp_glove_support_flag_changed=NULL;;
static  char gt1x_gtp_gesture_support_flag_changed=NULL;

#define CTP_GESTURE_FUNCTION_AUTHORITY_PROC 0777


static int gt1x_proc_gesture_data_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	int num = 0;
	if(*ppos) //CMD call again
		return 0;
    GTP_INFO("proc_gesture_data_read=%d\n",gt1x_gtp_gesture_value);
    num =  sprintf(buffer,"%c\n",gt1x_gtp_gesture_value);
	*ppos += num;
	return num;
}

static int gt1x_proc_gesture_data_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    sscanf(buffer,"%c",&gt1x_gtp_gesture_value);
    return count;
}

static int gt1x_proc_gesture_type_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	int num;
	if(*ppos) //CMD call again
	{	
    ...
    num = sprintf(buffer,"%s\n",gt1x_gtp_gesture_type);
	*ppos += num;
	return num;

}

static int gt1x_proc_gesture_type_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{

    return -1;
}

static int gt1x_proc_gesture_onoff_read(struct file *file, char *page, size_t count, loff_t *ppos)
{
	int num;
	if(*ppos) //CMD call again
		return 0;
    if(NULL==gt1x_gtp_gesture_support_flag_changed) 
		num = sprintf(page,"%c\n",gt1x_gtp_gesture_support_flag);
    else 
		num = sprintf(page,"%c\n",gt1x_gtp_gesture_onoff);
	*ppos += num;
	return num;

}

static int gt1x_proc_gesture_onoff_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    sscanf(buffer,"%c",&gt1x_gtp_gesture_onoff);
	GTP_DEBUG("gesture onoff %c", gt1x_gtp_gesture_onoff);
    gt1x_gtp_gesture_support_flag_changed=1;
    return count;
}

static int gt1x_proc_version_read(struct file *file, char *page, size_t count, loff_t *ppos)
{
	int num;
	if(*ppos) //CMD call again
		return 0;
...
static int gt1x_proc_version_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    return count;
}


static int gt1x_proc_gesture_coordinate_read(struct file *file, char *page, size_t count, loff_t *ppos)
{
	GTP_DEBUG("GTP gtp_ges count=%d, ppos=%d\n", count, *ppos);
	if(*ppos) //CMD call again
		return 0;
	memcpy(page,gt1x_gtp_gesture_coordinate,60);
	GTP_DEBUG("GTP gtp_ges 0x%02x\n", gt1x_gtp_gesture_coordinate[13]);
	memset(gt1x_gtp_gesture_coordinate, 0, 60);
	*ppos += 60;
	return 60;
}

static int gt1x_proc_gesture_coordinate_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    return count;
}

static const struct file_operations gt_gesture_var_proc_fops = { 
    .write = gt1x_proc_gesture_data_write,
    .read = gt1x_proc_gesture_data_read,
};

static const struct file_operations gt_gesture_type_proc_fops = { 
    .write = gt1x_proc_gesture_type_write,
    .read = gt1x_proc_gesture_type_read,
};

static const struct file_operations gt_gesture_onoff_proc_fops = { 
    .write = gt1x_proc_gesture_onoff_write,
    .read = gt1x_proc_gesture_onoff_read,
};

static const struct file_operations gt_version_proc_fops = { 
    .write = gt1x_proc_version_write,
    .read = gt1x_proc_version_read,
};

static const struct file_operations gt_coordinate_proc_fops = { 
    .write = gt1x_proc_gesture_coordinate_write,
    .read = gt1x_proc_gesture_coordinate_read,
};


void gt1x_Ctp_Gesture_Fucntion_Proc_File(void)
{
    ...
    {
        GTP_INFO("ctp_gesture_var_proc create failed\n");
    }
    ...
    {
        GTP_INFO("ctp_gesture_type_proc create failed\n");
    }
