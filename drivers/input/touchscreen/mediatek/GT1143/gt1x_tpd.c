...
#if CTP_ADD_HW_INFO
extern char tp_string_version[40];
extern int Module_ID;
#endif


#if GTP_GESTURE_WAKEUP
extern void gt1x_Ctp_Gesture_Fucntion_Proc_File(void);
...
#if GTP_GESTURE_WAKEUP
	input_set_capability(tpd->dev, EV_KEY, KEY_WAKE_GESTURE);
#endif
...
/* interrupt registration */
	...
	gt1x_esd_switch(SWITCH_ON);
		...
		gt1x_Ctp_Gesture_Fucntion_Proc_File();
#endif
...
static int tpd_event_handler(void *unused)
{
	...
	do {
		...
		if (ret < 0) {
			...
			GTP_DEBUG("IN TPD_EVENT_HANDLER");
#if !GTP_ESD_PROTECT
			gt1x_power_reset();
#endif
			goto exit_work_func;
...
static int tpd_local_init(void)
{
	...
	gpDMABuf_va = (u8 *) dma_alloc_coherent(NULL, IIC_DMA_MAX_TRANSFER_SIZE, &gpDMABuf_pa, GFP_KERNEL);
	if (!gpDMABuf_va) {
