...
u32 pinSet[2][8] = {
    ...
    //power ON
    if (On) {
		if (currSensorName && ((0 == strcmp(SENSOR_DRVNAME_OV8825_MIPI_RAW,currSensorName))||(0 == strcmp(SENSOR_DRVNAME_OV5648_MIPI_RAW,currSensorName))))
		{
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5645_MIPI_YUV,currSensorName)) )
       {
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_A5142_MIPI_RAW,currSensorName)))
		{
				...
				PK_DBG("[sym]kdCISModulePowerOn get in---  SENSOR_DRVNAME_A5142_MIPI_RAW cmpdn=%d,reset=%d \n",pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMRST]);
				PK_DBG("[ON_general 2.8V]sensorIdx:%d \n",SensorIdx);
				if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800,mode_name))
				{
					PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
